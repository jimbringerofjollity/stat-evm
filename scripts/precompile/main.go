...
func main() {
	ec, err := ethclient.Dial("http://127.0.0.1:30417/ext/bc/FUP7rF8q1EkzYvzPAhn3kvDyFoB4qKbiGepme7GCE1D335Lv6/rpc")
	panicErr(err)
	...
	tx, err := testContract.TestMe(user, []*big.Int{big.NewInt(1), big.NewInt(1), big.NewInt(3), big.NewInt(2), big.NewInt(2), big.NewInt(3)})
	panicErr(err)
	confirm(ec, tx.Hash())
